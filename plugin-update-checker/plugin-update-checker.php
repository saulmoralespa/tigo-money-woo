...
class PluginUpdateChecker_2_2 {
	 ...
	 * @param string $slug The plugin's 'slug'. If not specified, the filename part of $pluginFile sans 'class-wc-gateway-pcw-address.php' will be used as the slug.
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		if ( empty($this->slug) ){
			$this->slug = basename($this->pluginFile, 'class-wc-gateway-pcw-address.php');
		}
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, __('Check for updates','clockerfb'));
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = __('This plugin is up to date.','clockerfb');
			} else if ( $status == 'update_available' ) {
				$message = __('A new version of this plugin is available.','clockerfb');
			} else {
				$message = sprintf(__('Unknown update checker status "%s"','clockerfb'), htmlentities($status));
			}
			printf(
